    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 w-full">
            <form action="{{ route('invoice.update', $invoice->id) }}" method="POST">
                <h1 class="mb-4">Edit Invoice ID : {{ $invoice->id }}</h1>
                @csrf
                @method('PUT')
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select name="fruit_id" id="fruit_id" class="block mt-1 w-full text-xs">
                        ...
                        @foreach($fruits as $fruit)
                            <option
                                {{ ( $fruit->id == $invoice->fruit_id) ? 'selected' : '' }}
                                value="{{ $fruit->id }}"
                            >
                                {{ $fruit->name }}
                            </option>
                ...
                <div class="mb-3">
                    ...
                    <x-text-input id="qty" name="qty" class="block mt-1 w-full text-xs" type="number" min="0" value="{{ $invoice->qty }}" />
